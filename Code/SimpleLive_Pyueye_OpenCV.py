...
exposure_levels = [1000,3000,5000,10000]
exposure_levels = [ueye.double(x) for x in exposure_levels]
gain_levels = [0, 5, 10, 20, 50, 100]
...
nRet = ueye.is_Exposure(hCam, ueye.IS_EXPOSURE_CMD_SET_EXPOSURE, ueye.DOUBLE(5), ueye.sizeof(ueye.DOUBLE(5)))
if nRet != ueye.IS_SUCCESS:
    print("is_Exposure ERROR")
else:
    print("Changed exposure time to 5ms")

nRet = ueye.is_SetHardwareGain(hCam, 20, ueye.IS_IGNORE_PARAMETER, ueye.IS_IGNORE_PARAMETER, ueye.IS_IGNORE_PARAMETER)
if nRet != ueye.IS_SUCCESS:
    print("is_SetHardwareGain ERROR")
else:
    print("Changed gain to 20")
...
#---------------------------------------------------------------------------------------------------------------------------------------
    ...
    if key == ord('q'):
        break
    # change exposure time using 1~6
    for i in range(6):
        if key == ord(str(i + 1)):
            new_exposure = exposure_levels[i]
            nRet = ueye.is_Exposure(hCam, ueye.IS_EXPOSURE_CMD_SET_EXPOSURE, new_exposure, ueye.sizeof(ueye.DOUBLE()))
            if nRet != ueye.IS_SUCCESS:
                print("is_Exposure ERROR")
            else:
                print(f"Changed exposure time to {new_exposure}")
            # new_gain = gain_levels[i]
            ...
            # else:
            #     print(f"Changed gain to {new_gain}")
#---------------------------------------------------------------------------------------------------------------------------------------
...
print()
print("END")
print(f"time per frame: {(time.time() - start_time)/cnt} seconds")
